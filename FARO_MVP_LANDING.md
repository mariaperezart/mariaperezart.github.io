...
**Actualización — el primer contacto no es la landing, es el informe de auditoría.** Validado con el primer caso real trabajado (Clínica Veterinaria Centro León, ver `experimentos/EXP-003-pymes-leon/`): mostrar una landing ya construida sin que nadie la haya pedido puede leerse como "he rediseñado tu web sin que me lo pidieras". Un **informe de auditoría de recorrido de usuario** (5-10 minutos analizando la web real del negocio desde el punto de vista de su cliente, con hallazgos concretos de fricción — pop-ups, CTA débiles, pasos innecesarios, incertidumbre sobre qué pasará al contactar) es un ask mucho más fácil de aceptar (es gratis, es información, no exige decidir nada sobre un rediseño) y posiciona a FARO como quien **diagnostica** el negocio, no como quien vende páginas — coherente con `FARO_ESTRATEGIA_MARCA.md` ("FARO vende criterio, no ejecución técnica"). La landing pasa a ser la prueba de concepto que ilustra la sección de recomendación del informe, no el primer artefacto que se enseña. Plantilla de informe reutilizable en `propuestas/clinica-veterinaria-centro-leon/informe-auditoria.html` — mismo patrón para el resto del dataset de `EXP-003`.
...
## 9. Metodología de auditoría — ver `FARO_METODO_AUDITORIA.md`

**La auditoría de conversión ya no es una sección de este documento — es su propio método, con entidad propia.** Formalizado en `FARO_METODO_AUDITORIA.md` tras confirmarse con dos casos reales (veterinaria y dental) que el informe de auditoría gratuito no es solo "una forma de enseñar la landing sin parecer intrusivo" — es el caballo de Troya de todo el embudo comercial, con su propia lógica de escalabilidad entre verticales, sus 10 dimensiones de puntuación, y un menú de implementación que conecta cada hallazgo con un servicio concreto del catálogo. La landing de este documento pasa a ser una de las líneas de ese menú de implementación, no el punto de partida de la relación comercial.

No se duplica aquí el contenido — ver `FARO_METODO_AUDITORIA.md` para la metodología completa.

---

*Este documento no sustituye la posición de marca de FARO (`FARO_ESTRATEGIA_MARCA.md`) — la landing es una oferta de implementación dentro del embudo que abre `FARO_METODO_AUDITORIA.md`, no una redefinición de qué es FARO. Ver `FARO_MVP_CONCIERGE.md` para el mecanismo de seguimiento manual reutilizado aquí en la sección 4.2.*
